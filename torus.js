...
class Torus {
    ...
    lineCollide(line) {
        ...
        let matrix = new rotationalMatrix(this.alpha, this.beta, this.gamma);
        let ch = char;
        let minLen = 200;
...
const alpha = 2 * Math.PI / 80;
const beta = 2 * Math.PI / 80;
const gamma = 2 * Math.PI / 80;


function animation() {
    torus.rotate(alpha * Math.random(), beta * Math.random(), gamma * Math.random());
    let image = "";
